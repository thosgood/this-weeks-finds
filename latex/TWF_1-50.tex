...
\item
  ...
  in AMSLaTeX as
  \href{http://arxiv.org/abs/hep-th/9302028}{\texttt{hep-th/9302028}}.
\end{enumerate}
...
call this picture \textbf{one turn of the great wheel of time}\ldots.
...
\item
  ...
  preprint list, at \texttt{hep-lat@ftp.scri.fsu.edu}.)
\end{enumerate}
...
left is a Moebius strip. B. Take two projective planes, cut out a little
...
projective plane and a torus, so we have: \(3P = P + K = P + T\). D.
Show: \(4P = K + K = K + T\). E. Show: \((2n+1)P = P + nT\). F. Show:
\((2n+2)P = K + nT\).
...
\item
  ``Representation Theory of Analytic Holonomy \(C^*\) Algebras'', by
  Abhay Ashtekar and Jerzy Lewandowski
\item
  ``The Gauss Linking Number in Quantum Gravity'', by Rodolfo Gambini
  and Jorge Pullin (currently available as \texttt{gr-qc/9310025})
\item
...
\item
  ``Representation Theory of Analytic Holonomy \(C^*\) Algebras'', by
  Abhay Ashtekar and Jerzy Lewandowski, to appear in \emph{Knots and
  Quantum Gravity}, ed.~J. Baez, 42 pages, currently available as
...
\item
  ``An introduction to the moduli space of curves'', by Joe Harris, in
...
\textbf{clearly} nobody would want to switch from horses to automobiles
because then one would have to build \textbf{gas stations all over the
...
\href{http://www.grothendieckcircle.org/}{Sketch of a Program} (1983)
\end{quote}
...
Unfortunately, there are \textbf{several different ways} to set up
...
invariant under \textbf{all diffeomorphisms of spacetime}. (So maybe the
...
function.) The answer is no --- \textbf{except} if \(n = 4\), where
