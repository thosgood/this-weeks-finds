...
algebra of E8. The bracket of two guys in \mathfrak{so}(16) is defined as usual,
the bracket of a guy in \mathfrak{so}(16) and a guy in S+ is defined to be the
...
\mathfrak{so}(16) ⊗ S+\to S+

to get a map

S+ ⊗ S+\to \mathfrak{so}(16).
...
with James Dolan. Stasheff invented a notion of "A~\infty~ space", which is
a pointed topological space with a product that is associative up to
homotopy which satisfies the pentagon identity up to... etc. Any A~\infty~
...
the group of length-one elements of R (i.e. Z/2) we get RP^\infty^, as we
expect, since
...
-   The Lie algebra E6 has a subalgebra of maximal rank isomorphic to
    \mathfrak{so}(10) ⊕ u(1). The quotient space is 32-dimensional - twice the
...
-   The Lie algebra E7 has a subalgebra of maximal rank isomorphic to
    \mathfrak{so}(12) ⊕ su(2). The quotient space is 64-dimensional - twice the
