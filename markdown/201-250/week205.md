...
In \"[week199](week199.html)\" I explained the analogue of a \"point\"
...
\"[week199](week199.html)\" before charging ahead into the beautiful
wilderness.
...
other field k\', we can compose them to get a homomorphism f \': R →
...
tell when f: R → k and f \': R → k\' are related by such a flavor
changing process, or a sequence of such processes. You just see if they
have the same kernel! The \"kernel\" of f: R → k is the subset of R
...
\"[week199](week199.html)\" we saw that except for the prime ideal
...
In \"[week172](week172.html)\" I discussed Tarski\'s \"high school
...
explained in \"[week200](week200.html)\".
...
\"[week172](week172.html)\", which gives a very detailed account of this
problem:
...
\"flavor-changing operations\" coming from homomorphisms k → k\' as the
morphisms.
...
in F~3~.) However, these two k-points are isomorphic, since there\'s an
...
points Z\[i\] → F~9~ sitting over the unique point Z → F~9~ whose kernel
is that prime ideal.
