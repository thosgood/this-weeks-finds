...
number, as described in \"[week216](week216.html)\".
...
\"[week216](week216.html)\". These are usually called \"Dedekind zeta
...
It\'s not true that ζ(s) = ζ(1-s), but something similar is true, where
...
to itself, and the Riemann Hypothesis says all the ζ zeros in the
...
We start with the function ζ(2s):
...
where a,b,c,d form a 2×2 matrix of integers with determinant 1. These
maps form a group called PSL(2,Z), or the \"modular group\".

A function that transforms simply under this group and doesn\'t blow up
in nasty ways is called a \"modular form\". In
\"[week197](week197.html)\" I gave the precise definition of what counts
...
theory. So, please either reread \"[week197](week197.html)\" or take my
...
subgroup of PSL(2,Z), namely that generated by
...
Indeed, for any discrete subgroup G of PSL(2,Z), functions that
...
a~1~ 1^-s^ + a~2~ 2^-s^ + a~3~ 3^-s^ + a~4~ 4^-s^ + \....
...
series! If we take the monoid algebra of (N,×,1), we get the algebra of
Dirichlet series!
...
(N,+,0) would give us the algebra of polynomials, while (N,×,0) would
give us the algebra of \"Dirichlet polynomials\".)

Of course, algebraically we can combine these structures. (N,+,×,0,1) is
...
> complex numbers a~1~, a~2~,\...,a~n~,\..., with
>
> a~n~ = O(n^r ^),
>
...
> an example. Take a sequence of complex numbers a~1~, a~2~,\... which
...
> other poles, and one could even argue that the a~i~ \"came from an
> automorphic form\".
>
...
> can build all modular forms of level 1 from the Eisenstein series E~4~
...
> of K that fix Q. Say f has degree n and n roots z~1~, z~2~, \... ,
> z~n~. Then any automorphism of K fixes f, so it permutes the roots of
> f. So we get an embedding
>
> Gal(K/Q) → S~n~
>
> where S~n~ is the symmetric group on the set z~1~, z~2~, \... , z~n~.
...
> always\-\--if there are subtle relations amongst the z~i~ with
...
> and the Galois group cannot possibly be all of S~4~. In fact one can
> check that the Galois group is D~8~, the dihedral group with 8
...
> extension of degree 2, with Galois group S~2~ which is abelian.
>
...
> are Galois over Q, then the obvious restriction map Gal(K/Q) →
