...
into the stratosphere of n-categories for general n, or even n = ∞,
...
For more references, try \"[week170](week170.html)\".
...
about ∞-groupoids - that is, ∞-categories where all the j-morphisms have
weak inverses.
...
I can cook up an ∞-category which has:
...
The interesting thing about the above ∞-category is that it\'s an
\"(∞,1)-category\", meaning that all its j-morphisms are weakly
...
We can define \"(∞,k)-categories\" for any k in the same way, and we see
that (∞,0)-categories are just ∞-groupoids. So, the Homotopy Hypothesis
...
Thursday was all about (∞,1)-categories. The first talk was by Mike
Shulman, who gave a nice intuitive treatment of André Joyal\'s approach
to (∞,1)-categories.

In 1957, Daniel Kan figured out a nice way to describe ∞-groupoids as
...
(∞,1)-categories as simplicial sets with some more general property. In
...
these (∞,1)-categories, which he calls \"quasicategories\". He is
...
Next Julie Bergner compared different approaches to (∞,1)- categories. I
...
(∞,1)-category. But then she showed these definitions weren\'t really so
different! For each definition, she constructed a model category of all
(∞,1)-categories of that type. And then, she sketched the proof that all
these model categories were \"Quillen equivalent\".

For details, listen to her talk or try this paper:

13\) Julie Bergner, A survey of (∞, 1)-categories, available as
...
\"[week223](week223.html)\"). In the afternoon, Urs Schreiber described
