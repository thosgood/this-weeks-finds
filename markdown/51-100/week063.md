...
roots are all at angles π/n from each other, where n \> 1 is an integer.
...
otherwise, if they are at an angle π/n from each other, we draw a line
...
Recall that I~m~ is the symmetry group of the of regular m-gon, while
...
For example, the symmetry group of the dodecahedron is H~3~, while that
of its 4-dimensional relative is H~4~.
...
side of a regular pentagon, (√5 + 1)/2, is called the golden ratio? Or
...
lattice having H~3~ as its symmetries.
...
A~n~, B~n~, D~n~, E~6~, E~7~, E~8~, F~4~, and G~2~

(and those corresponding to disjoint unions of these diagrams). In other
words, the symmetry groups of the pentagon (I~5~), the heptagon and so
on (I~m~ with m \> 6), and the dodecahedron and its 4-dimensional
relative (H~3~ and H~4~) are ruled out.
...
the circle (with addition mod 2π), and the groups SO(n) and SU(n)
discussed in "[week61](week61.html)". These groups are incredibly
...
G. Oh yes, and C. I have to tell you what happend to C.
...
algebra that satisfies those axioms. For example, you could take n x n
...
symmetries, but for B there are two. Recall the diagram B~n~ looks like:

              4
    o---o---o---o

with n dots. And recall that the dots correspond to "roots", which in
the present context are vectors in Lie(T). Now it turns out that
...
two diagrams, which we call B~n~ and C~n~. One of them has the root at
...
which is perfectly symmetrical. So folks usually consider C~n~ only for
...
theorists think the symmetry group of nature is E~8~, the largest
