...
[quant-ph/9907007](http://arxiv.org/abs/quant-ph/9907007).
...
though it seems like a lot of fun (see "[week40](week40.html)"). Thus
...
Tracts in Theoretical Computer Science 7, Cambridge U. Press, 1989. Also
available at <http://www.cs.man.ac.uk/~pt/stable/Proofs+Types.html>
...
4) Eric Goubault, Schedulers as abstract interpretations of
higher-dimensional automata, in Proc. PEPM '95 (La Jolla), ACM Press,
1995. Also available at
<http://www.di.ens.fr/%7Egoubault/GOUBAULTpapers.html>

Eric Goubault and Thomas Jensen, Homology of higher-dimensional
automata, in Proc. CONCUR '92 (New York), Lecture Notes in Computer
Science 630, Springer, 1992. Also available at
<http://www.di.ens.fr/%7Egoubault/GOUBAULTpapers.html>
...
6) Craig C. Squier, Word problems and a homological finiteness
condition for monoids, Jour. Pure Appl. Algebra 49 (1987), 201-217.

Craig C. Squier, A finiteness condition for rewriting systems, revision
by F. Otto and Y. Kobayashi, to appear in Theoretical Computer Science.
...
will get too messy if I do that, so I won't. Then, whenever we have ab
= c in the monoid, we stick in a 2-simplex, which we can visualize as a
triangle like this:
...
only on the monoid, so we can call them H~k~(M). If we have a
presentation of M with only finitely many generators, we can build KM
using 1-simplices only for those generators, and it follows that H~1~(M)
...
we can build KM using only finitely many 2-simplices, so H~2~(M) is
...
can build KM using only finitely many 3-simplices, so H~3~(M) is
finitely generated! What's nice about this is that homological algebra
gives an easy way to compute H~k~(M) given a presentation of M, so in
...
H~3~(M) is too big. Examples of this, and many further details, appear
in Lafont's work:
...
Also available at <http://iml.univ-mrs.fr/~lafont/publications.html>
...
(Of course, folks who read "[week38](week38.html)" will know that
