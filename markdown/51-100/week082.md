...
the nicest --- let's call it $C_n$ --- is just the associative algebra over
...
How about the $C_n$ for larger values of $n$? Well, here is a little table
up to $n = 8$:
...
algebras $C_n$ for $n$ up to 8. You have to find the "isomorphism" --- the
...
Namely, $C_{n+8}$ consists of $16\times 16$ matrices with entries in $C_n$! For
