...
2×2 \"density matrix\" - a self-adjoint matrix with nonnegative
...
algebra approach described in \"[week162](week162.html)\". Barnum cites
...
Some nice examples of Jordan algebras are the 2×2 self-adjoint matrices
...
Back in \"[week162](week162.html)\" I explained how these examples are
...
2×2 self-adjoint complex matrices - let\'s call it h~2~(C) - is
...
mentioned in \"[week209](week209.html)\", taking R to be the real
numbers. Here:
...
\"exchangeable\" density operator ρ~n~ on H^⊗\ n^ - that is, one such
that for each N ≥ n, there\'s a density operator ρ~N~ on H^⊗\ N^ which
1) is invariant under permutations in S~N~ and 2) has ρ as its marginal,
meaning that

Tr (ρ~N~) = ρ~n~

where Tr is the partial trace map sending operators on H^⊗\ N^ to
operators on H^⊗\ n^. Then, ρ~n~ is a mixture of density matrices of the
form ρ ⊗ \... ⊗ ρ: a tensor product of n copies of a density matrix on
H.
