...
We\'ve seen Enceladus once before, in \"[week231](week231.html)\". I
...
back in \"[week269](week269.html)\": they\'re cage-shaped ice crystal
...
PSL(n,q) is the group of n × n matrices with determinant 1 having
entries in the field with q elements, mod multiples of the identity
matrix. Of the groups listed above, only A~4~ is not simple. I included
...
\"[week79](week79.html)\", to see how these stories are related to
...
\"measure\" on a measurable space assigns a number between 0 and +∞ to
...
We can also form L^∞^(X), the space of equivalence classes of bounded
measurable complex functions on X, where we identify functions that
agree except on a set of measure zero.

L^∞^(X) is an algebra where we multiply functions pointwise. We can
...
\"[week175](week175.html)\", so I won\'t do it again here.

L^∞^(X) is a very special sort of von Neumann algebra, namely a
...
operators on a *separable* Hilbert space is isomorphic to L^∞^(X) for
...
the topology where a bunch of measures μ~i~ converge to μ if for every
...
\"[week89](week89.html#tale)\", so I won\'t do it again, but here it
...
Ted Nitz calculated the odds that a pair of elements in A~4~ generates
