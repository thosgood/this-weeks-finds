...
Martian dunes which graced \"[week262](week262.html)\".
...
\"[week102](week102.html)\". Very roughly, the nth homotopy group of a
...
π~11~(S^2^) = Z/2\
π~12~(S^2^) = Z/2 × Z/2\
π~13~(S^2^) = Z/2 × Z/2 × Z/3\
π~14~(S^2^) = Z/2 × Z/2 × Z/4 × Z/3 × Z/7\
π~15~(S^2^) = Z/2 × Z/2\
...
So, what people often do first is take the limit of π~n+k~(S^n^) as n →
∞, getting what\'s called the kth \"stable\" homotopy group of spheres.
...
but if we work \"at the prime 2\" we just see the Z/2 × Z/2 × Z/4.
...
elements are n-strand braids. For example, here\'s an element of B~3~:
...
\"[week233](week233.html)\". But anyway, we count two braids as the same
...
As I hinted in \"[week261](week261.html)\", these surface braid groups
...
as explained in \"[week151](week151.html)\":

\... → π~n~(S^1^) → π~n~(S^3^) → π~n~(S^2^) → π~n-1~(S^1^) → \...
...
explained this repeatedly here - try \"[week125](week125.html)\" for a
...
quantize a black hole - see the end of \"[week112](week112.html)\" for
more details.
...
by taking the nth tensor power of L - let\'s call that L^⊗n^. We can
multiply a section of L^⊗n^ and a section of L^⊗m^ to get a section of
L^⊗(n+m)^. So, all these spaces of sections we\'re studying fit together
...
of all the line bundles L^⊗n^ will have one generator of degree 1, one
...
\"[week13](week13.html)\" and \"[week261](week261.html)\" for details.)
...
hence a section of L^⊗2^. And y should be cubic, hence a section of
L^⊗3^. If L has first Chern number 1, I think we\'re in business.
...
combinatorial description of π~3~(S^2^).
