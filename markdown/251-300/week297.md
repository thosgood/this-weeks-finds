...
classifying space of the groupoid of finite sets is Ω^∞^S^∞^ - see
\"[week199](week199.html)\" if you don\'t know what I\'m talking about
...
φ~i~ = φ~j~ for all i,j implies Q(φ) = 0

and also satisfy the \"Markov property\":

Q(φ) ≥ Q(ψ)

when ψ~i~ is the minimum of φ~i~ and 1. This characterization is
...
in \"[week289](week289.html)\". And then we can study the heat equation
...
P(φ~1~, φ~2~) = min~φ~3~~ Q(φ~1~, φ~2~, φ~3~)
...
Q(φ) = ∑~i,j~ c~ij~ (φ~i~ - φ~j~)^2^ + ∑~i~ c~i~ φ~i~^2^

where c~ij~ and c~i~ are nonnegative numbers.
...
when ψ~i~ is the minimum of φ~i~ and 1.
...
include quadratic forms where the cross-terms φ~i~ φ~j~ have positive
...
> -   The dagger of a subspace of T\*V × T\*W is the corresponding
>     subspace of T\*W × T\*V.
...
Lagrangian subspaces of T\*R^m^ × T\*R^n^, but only those lying in the
subset C(R^m^ × R^n^). So, let\'s try:

> **Claim:** there is a dagger-compact category where:
>
> -   An object is a natural number.
> -   A morphism S: m → n is a point in C(R^m^ × R^n^).
> -   We compose morphisms using composition of relations.
> -   The tensor product is given by direct sum.
> -   The symmetry is the obvious thing.
> -   The dagger of a point in C(R^m^ × R^n^) is the corresponding point
>     in C(R^n^ × R^m^).
...
> about potentials at terminals, replace \"the potential at T~i~\" with
> \"the potential difference between T~i~ and G\" (or equivalently but
...
is dead\', \'linear system theory is dead\', \'H~∞~ is dead\'. Good
