...
components. As we\'ve seen starting in \"[week288](week288.html)\",
...
\"[week294](week294.html)\", and the x\'s mark the input and output.
...
-   A morphism Q: V → W is a nonnegative quadratic form Q on V+W.

How do we compose these morphisms? Using the principle of minimum power!
Given morphisms P: U → V and Q: V → W, we define their composite QP: U →
W by
...
the reason is clear. The identity morphism 1: R → R should correspond to
...
\"[week294](week294.html)\". But what\'s the corresponding quadratic
...
sum V+W, while the tensor product of morphisms Q:V → W and Q\':V\' → W\'
...
that we can take any circuit Q: V → W built from resistors:
...
to obtain a new circuit Q^†^: W → V. Mathematically this operation is
defined as follows:
...
sense explained back in \"[week89](week89.html)\".
...
linear subspace of the cartesian product V × V\*. But V × V\* is better
...
\"Lagrangian\" subspace of T\*V = V × V\*, meaning a maximal subspace on
which ω vanishes.
...
-   An object is a finite-dimensional real vector space.
-   A morphism S: V → W is a Lagrangian subspace of T\*(V+W).
...
before, the Lagrangian subspace of T\*V × T\*W describes the potentials
