...
\"[week272](week272.html)\" and \"[week273](week273.html)\", and then
Phoebe and Iapetus in \"[week281](week281.html)\". Someday we should
...
(see \"[week273](week273.html)\"). It goes like this. Once upon a time
...
\"[week191](week191.html)\", so if the word \"operad\" fills you with
...
More precisely, a linear operad O consists of a vector space O~n~ for
...
Here we are feeding the outputs of n operations g~1~,..,g~n~ into the
...
We demand that this give an action of the permutation group on the O~n~.
...
O is a vector space V together with maps that turn elements of O~n~ into
...
\"representation\" of O on V, since the idea is that elements of O~n~
...
and call its kernel I. I~n~ is the space of n-ary operations for
...
interesting case is I~2~. Assoc~2~ is 2-dimensional, with this basis:

(x,y) \|→ xy\
(x,y) \|→ yx

and I~2~ is 1-dimensional, with this basis:

(x,y) \|→ xy - yx

since this is what\'s zero for commutative algebras. The quotient
Assoc~2~/I~2~ is the same as Comm~2~: it\'s a 1-dimensional space, and
...
since the homomorphism Assoc → Comm is onto, we have
...
o (g~1~, \..., g~n~) where f is in J and at least one of the g~i~\'s in
...
graded vector space. So, the total dimension of Poisson~n~ equals the
dimension of Assoc~n~, which is n factorial.
...
\"[week185](week185.html)\", \"[week190](week190.html)\", and
\"[week102](week102.html)\", but not in the linear case. It\'s no big
deal: where we used cardinalities before, now we use dimensions! We get
these generating functions:

\|Lie\|(x) = ∑~n~ dim(Lie~n~) x^n^/n!

\|Assoc\|(x) = ∑~n~ dim(Assoc~n~) x^n^/n!

                = ∑~n~ x^n^

                = 1/(1-x)

\|Comm\|(x) = ∑~n~ dim(Comm~n~) x^n^/n!
...
Comm(Lie(V)) ≅ &oplus~n~ Poisson~n~ ⊗ V^⊗n^

To get a basis of Poisson~n~, it\'s therefore enough to consider
...
\"[week212](week212.html)\". But I didn\'t fully integrate this stuff
into the story.
...
\"[week220](week220.html)\", but you\'ll notice that this only talks
...
>
> (Given a filtration R = R~0~ ⊇ R~1~ ⊇\..., e.g. by powers of I, you
> can look at the subring of R\[t\] that has t^n^ R~n~ in the nth degree
