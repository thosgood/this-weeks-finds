...
In \"[week289](week289.html)\", I listed two examples of systems where
...
five basic 1-ports in \"[week290](week290.html)\": resistors,
...
of things. For example, i × i = -1 lets us introduce the \"imaginary\"
...
involving q~1~,q\'~1~,p~1~,p\'~1~, q~2~,q\'~2~,p~2~,p\'~2~, and perhaps
...
> \"[week290](week290.html)\" I listed the other three, namely the
> resistor:
>
> p\' = f(q\')
>
...
combinations of vertices. We also get a space C~1~ of \"1-chains\":
...
explained in \"[week290](week290.html)\".
...
the nth tensor power of x is drawn as a list of n x\'s. So, for example,
...
> also, there is an analogue for atomic capacitance = d^2^E/dq^2^ and
...
>
> bond capacitance × charge transfer variables = pairs of voltage
> differences
>
...
> combinations of bond variables like (1 → 2) + (2 → 3) + (3 → 1) must
...
respect to the number of electrons. And when he says d^2^E/dq^2^ is a
...
\"soft\" ones are big and strongly polarizable. The bigger d^2^E/dq^2^
...
stretch has a big value of d^2^E/dq^2^ where E is energy and q is how
