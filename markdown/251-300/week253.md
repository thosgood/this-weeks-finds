...
G = (\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1))/(Z/6)
...
of S(\mathrm{U}(3) \times \mathrm{U}(2)) as the true gauge group of the Standard Model! Maybe
...
of this group. \mathrm{SU}(5) has an obvious representation on C^5, via matrix
multiplication. So, it gets a representation on the exterior algebra
Λ(C^5). If we restrict this from \mathrm{SU}(5) to S(\mathrm{U}(3) \times \mathrm{U}(2)), we get
...
different irreducible representations of S(\mathrm{U}(3) \times \mathrm{U}(2)), which are
...
The group \mathrm{SU}(n) acts on the exterior algebra Λ(C^n). So, its Lie
...
Λ(C^n) = Λ^even^(C^n) \oplus  Λ^odd^(C^n)
...
Well, S~10~ is defined to be a real representation of \mathfrak{so}(10). But, it
...
The last two also consider the gauge group "G(224)", meaning \mathrm{SU}(2) \times
...
Using S(\mathrm{U}(3) \times \mathrm{U}(2)) and demanding anomaly cancellation, we
...
> the unit complex numbers, \mathrm{U}(1), on S~10~^+^ which commutes with the
> action of \mathfrak{so}(10).

But in fact, \mathfrak{so}(10) preserves *two* complex structures on S~10~. This is
