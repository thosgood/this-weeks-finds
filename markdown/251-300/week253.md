...
where g is a 3×3 block and h is a 2×2 block. For obvious reasons, I call
this group

S(U(3) × U(2))

If you want some intuition for this, think of the 3×3 block as related
to the strong force, and the 2×2 block as related to the electroweak
force. A 3×3 matrix can mix up the 3 \"colors\" that quarks come in -
red, green, and blue - and that\'s what the strong force is all about.
Similarly, a 2×2 matrix can mix up the 2 \"isospins\" that quarks and
leptons come in - up and down - and that\'s part of what the electroweak
force is about.

If this isn\'t enough to make you happy, go back to
\"[week119](week119.html)\", where I reviewed the Standard Model and its
...
of S(U(3) × U(2)) as the true gauge group of the Standard Model! Maybe
...
Λ(C^5^). If we restrict this from SU(5) to S(U(3) × U(2)), we get
...
different irreducible representations of S(U(3) × U(2)), which are
...
> in the Standard Model, as long as we remember how S(U(2) × U(3)) sits
...
to break the symmetry from Spin(10) down to S(U(3) × U(2)), we need to
...
which have the charming names G~2~, F~4~, E~6~, E~7~, and E~8~. These
are all related to the octonions. G~2~ is just the automorphism group of
the octonions. The other 4 are closely related to each other - thanks to
the \"magic square\" of Rosenfeld, Freudenthal and Tits.

I talked about the magic square a bit in \"[week106](week106.html)\" and
\"[week145](week145.html)\", and much more here:
...
e~6~ ≅ so(C ⊕ O) ⊕ (C ⊗ O)^2^ ⊕ Im(C)

e~7~ ≅ so(H ⊕ O) ⊕ (H ⊗ O)^2^ ⊕ Im(H)

e~8~ ≅ so(O ⊕ O) ⊕ (O ⊗ O)^2^

Here f~4~, e~6~, e~7~ and e~8~ stand for the Lie algebras of the compact
...
on this space. This is just the obvious complex structure on (C ⊗ O)^2^,
...
The symmetric space I\'m talking about now, namely E~6~/(Spin(10) ×
...
Another nice fact is that (C ⊗ O)P^2^ is one of the Grassmannians for
E~6~. I explained this general notion of \"Grassmannian\" back in
\"[week181](week181.html)\", and you can see this 16-dimensional one in
...
and writing the linear transformations in so(O ⊕ O) as two 8×8 blocks
living in so(O), together with an off-diagonal block living in O ⊗ O.
This gives

e~8~ = so(O) ⊕ so(O) ⊕ (O ⊗ O)^3^

Then, he wants to use each of the three copies of O ⊗ O to describe one
...
The last two also consider the gauge group \"G(224)\", meaning SU(2) ×
SU(2) × SU(4).
...
A Standard Model from the E~8~ × E~8~ heterotic superstring,
...
octonionic projective plane is not really E~6~/(Spin(10) × U(1)); it\'s
...
Using S(U(3) × U(2)) and demanding anomaly cancellation, we
...
> >     of rank 1.
>
> Since (C ⊗ O)P^2^ = E~6~/(SO(10) × SO(2)) is rank 2, it is NOT
> isotropic.
...
> (C ⊗ O)^2^, or if you prefer, Λ^even^(C^5^). So, there\'s an action of
