...
In \"[week257](week257.html)\" and \"[week258](week258.html)\" I talked
...
Back in \"[week223](week223.html)\" I showed my favorite example of a
...
on the Riemann sphere. In \"[week201](week201.html)\", I explained how
...
I explained more about elliptic curves in \"[week13](week13.html)\" and
\"[week125](week125.html)\". Lurking behind this, there\'s a lot of
...
polynomial equation. For example, in \"[week20](week20.html)\" I talked
...
\"[week205](week205.html)\", \"[week216](week216.html)\" and
\"[week218](week218.html)\", but there\'s much more to say about what\'s
...
\"[week257](week257.html)\". It seems these analogies are doomed to
...
I gave a lot of reasons in \"[week183](week183.html)\",
\"[week184](week184.html)\", \"[week185](week185.html)\",
\"[week186](week186.html)\" and \"[week187](week187.html)\", but let me
rapidly summarize.
...
These examples can be generalized. In \"[week187](week187.html)\" I
...
reduce to \"Coxeter groups\" as q → 1. And, if you mark some dots on a
Dynkin diagram you get a \"flag variety\" on which your algebraic group
acts\... but as q → 1, this reduces to a finite set on which your
Coxeter group acts.
...
F~1~ are just finite sets, what should *vector spaces* over F~1~ be?
...
Soulé tries to define \"algebraic varieties\" over F~1~, namely curves
...
The theme of zeta functions - see \"[week216](week216.html)\" - is
deeply involved in this business. For more, try these papers:

8\) N. Kurokawa, Zeta functions over F~1~, Proc. Japan Acad. Ser. A
Math. Sci. 81 (2006), 180-184.

9\) Anton Deitmar, Remarks on zeta functions and K-theory over F~1~,
available as
...
10\) Anton Deitmar, Schemes over F~1~, available as
[arXiv:math/0404185](http://arXiv.org/abs/arXiv:math/0404185).

F~1~-schemes and toric varieties, available as
...
For Deitmar, the field with one element, F~1~, is just the commutative
monoid with one element, namely 1. A \"vector space over F~1~\" is just
...
\"[week199](week199.html)\" and \"[week205](week205.html)\", but let me
...
2.  If U is covered by a bunch of open sets U~i~, and we have a function
    f~i~ in each O(U~i~), such that f~i~ and f~j~ agree when restricted
    to the set U~i~ ∩ U~j~, then there\'s a unique function f in O(U)
    that restricts to each of these functions f~i~.
...
to \"[week257](week257.html)\", we might understand this if we could see
...
The problem is, F~1~ isn\'t really a field. In Deitmar\'s approach,
...
He starts by defining a \"commutative ring over F~1~\" to be simply a
commutative monoid. The simplest example is F~1~ itself.
...
Deitmar shows how to build a scheme over F~1~ called the \"projective
line\". The projective line over C is just the Riemann sphere. The
projective line over F~1~ has just two points (or more precisely, two
...
Deitmar also shows how to relate schemes over F~1~ to the usual sort of
schemes.
...
F~1~\... and conversely. He says an ordinary scheme is \"defined over
F~1~\" if it arises in this way from a scheme over F~1~.
...
Toen describes interesting relations between algebra over F~1~ and
...
over F~1~! This is fascinating\... I don\'t understand it, but I want
to.
...
g(f(x~1~,x~2~), f(x~3~,x~4~), f(x~5~,x~6~)) = f(g(x~1~,x~2~,x~3~),
g(x~4~,x~5~,x~6~))
...
with r~i~ in R. Here an example of a binary operation is addition:
...
(In the calculations I just did, we can either think of the x~i~ as
