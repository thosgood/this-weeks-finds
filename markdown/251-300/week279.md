...
conjugate each entry. Let\'s use h~2~(K) to mean the set of hermitian
2×2 matrices with entries in K.

Then a nice thing happens: h~2~(K) is the same as (n+2)-dimensional
Minkowski spacetime! To see this, note that any guy in h~2~(K) has this
form:
...
Since K has dimension n, h~2~(K) has dimension n+2. And check out its
determinant:
...
-   h~2~(R) is 3-dimensional Minkowski spacetime.
-   h~2~(C) is 4-dimensional Minkowski spacetime.
-   h~2~(H) is 6-dimensional Minkowski spacetime.
-   h~2~(O) is 10-dimensional Minkowski spacetime.
...
happy when spacetime is secretly h~2~(K)?
...
10 are nicely described by elements of h~2~(K). And it turns out that
...
Minkowski spacetime back in \"[week196](week196.html)\". So go there if
...
all these kinds back in \"[week93](week93.html)\", and I don\'t want to
...
born for! Just take your matrix in h~2~(K), multiply your spinor in K^2^
...
Mathematicians would write it as the operation that takes A in h~2~(K)
and ψ in K^2^ and multiplies them to get Aψ in h~2~(K).
...
operation that takes two spinors, say ψ in K^2^ and φ in K^2^, and
creates a vector in h~2~(K) that I\'ll call ψ·φ. If you want the
...
- leaving many details to \"[week93](week93.html)\".
...
corresponds to the operation where we take a guy in h~2~(K) and a guy in
K^2^ and multiply them to get a guy in h~2~(K)?
...
described by an element of h~2~(K) - with K = C, since we live in
...
the \"obvious\" way for an element of h~2~(K) to act on K^2^, there is a
less obvious way that involves the \"traced-reversed\" form of a 2×2
hermitian matrix:
...
∞-groupoids. For that you\'ll have to read these:
