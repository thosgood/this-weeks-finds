...
this; for now see \"[week66](week66.html)\" if you have no idea what
...
some of which are listed in \"[week124](week124.html)\". Does anyone see
the connection?
...
Barrett-Crane model! Here the gauge group is Spin(4) = SU(2) × SU(2),
...
where F is the curvature of the gauge field, ψ is a spinor field
transforming under some representation of SU(N), ψ^†^ is my feeble
substitute for a ψ with a bar over it, and D is the covariant Dirac
operator. If we write this out a bit more explicitly, it\'s

tr((dA + \[A,A\]) ∧ ∗(dA + \[A,A\]) + ψ^†i^ (d~a~ + A~a~) Γ^a^~ij~ ψ^j^

where A is the gauge field. But now let\'s assume A and ψ are constant
as functions on space, and that ψ transforms in the adjoint
representation of su(N). This amounts to saying that A lies in su(N) ⊗
R^n^, where n is the dimension of spacetime, and that ψ lies in su(N)
...
tr(\[A~a~,A~b~\] \[A^a^,A^b^\]) + ψ^†i^ \[A~a~, Γ^a^ij ψ^j^\]

which is the Lagrangian for the IKKT model.

Now the idea is that as N → ∞, this sort of theory can reduce to string
theory on some n-dimensional spacetime manifold\... but not necessarily
any fixed manifold.

It will be no surprise to readers of \"[week93](week93.html)\" and
\"[week104](week104.html)\" that this model is supersymmetric when the
...
case, we can think of both A and ψ as big fat matrices of octonions!
...
explained this in \"[week121](week121.html)\".
...
> Γ~0~(n^2^), consisting of 2×2 integer matrices of determinant 1 whose
...
> (a,d) mod n is the condition that ad-n^2^bc=1, and thus that ad=1 mod
...
> \-- in which case we have a normal subgroup of SL~2~(Z) \[the group of
> 2×2 integer matrices of determinant 1\], so in particular the
...
> modular forms and functions for congruence subgroups of SL~2~(Z). If
