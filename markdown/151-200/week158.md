...
\"[week118](week118.html)\"):

    L = - (1/2k2) e R  
        - (1/2) e ψ*M ΓMNP DN[ω + ω')/2] ψP 
        - (1/48) e F2
        - (√2k/384) e (ψ*M ΓMNPQRS ψS + 12 ψ*N ΓPQ ψR) (F + F')NPQR 
        - (√2k/3456) εM1 ... M11 FM1 ... M4 FM5 ... M8 AM9 ... M11
...
B\) a field ψ on this manifold which takes values in the real spin-3/2
...
why people write it as ψ~N~: the subscript indicates that we\'ve got
...
reread \"[week93](week93.html)\". You\'ll see that by Bott periodicity,
...
of Majorana spinors in 11d spacetime is R^2^ tensor R^16^ = R^32^. The
...
where the \"curvature\" or \"field strength tensor\" F is given by F =
dB.
...
Ricci scalar cooked up from the spin connection. \"ψ~N~\" is the
gravitino field, and physicists write the inner product on spinors as
\"ψ\*~N~ ψ^N^\", where the star should really be a bar: a line drawn
over the letter ψ. \"A\" is the 3-form field and \"F\" is the field
...
The middle term is, up to a constant, just what I\'d call \"F \^ \*F\":
the Lagrangian for the 3-form analog of Maxwell\'s equations.

The last term is, again up to a constant, just what I\'d \"F \^ F \^
A\". This is an 11-dimensional analog of the Chern-Simons term F \^ A
...
The \"Γ\" gadgets are antisymmetrized products of γ matrices, i.e.
Clifford algebra generators. I don\'t mind that. It\'s the stuff
involving ω\' and F\' that confuses me. \"ω\" is just a name for the
spin connection, so D~v~\[ω\] would mean \"covariant differentiation
with respect to the spin connection\". But instead of using that, we use
D~v~\[(ω + ω\')/2\], where ω\' is the \"supercovariantization\" of the
...
and Bott periodicity (see \"[week61](week61.html)\" and
\"[week105](week105.html)\"), it\'s also very natural to think of these
Majorana spinors as pairs of octonions.

Second, the Chern-Simons-like term F \^ F \^ A in 11d supergravity is
akin to the 3d Chern-Simons Lagrangian F \^ A. But this relation is a
...
\"[week25](week25.html)\" and \"[week151](week151.html)\", but the basic
...
\"[week118](week118.html)\", you can only write down Lagrangians for
...
>     > That's why people write it as ψN: the subscript indicates that we've
...
>
> 1 ⊗ 1/2 = 3/2 &oplus 1/2 (where ⊗ is tensor product, ⊕ is direct sum)
>
...
> ψ_M with a Γ\^M matrix and summing of course on the vector index. The
...
>     >
>     > L = - (1/2k^2) e R
>     >     - (1/2) e psibar_M Γ^{MNP} D_N[(ω + ω')/2] ψ_P
>     >     - (1/48) e F^2
>     >     - (sqrt(2)k/384) e
>     >       (psibar_M Γ^{MNPQRS} ψ_S + 12 psibar^N Γ^{PQ} ψ^R)
>     >       (F + F')_{NPQR}
>     >     - (sqrt(2)k/3456) ε^{M1 ... M11}
...
> F/\\\*F. Cool! Now, we can go further. A is a 3-form, so F is a
> 4-form, then \*F is a 11-4=7-form, then it should be the field
> strength tensor of some 6-form potential, dA\_(6)=\*F, But a 6-form is
...
> \_infinite\_ number of different counter terms. In order to fix all
...
> infinite but with \_fixed\_ relative coefficients. So there is still
...
> ψ\^4 theory is not renormalizable and is a nice theory of weak
...
> one loop counterterm of the form R\^4. This is just an infinity in
...
> positions (remember, for anticommuting variables ψ\^a φ_a = - ψ_a
> φ\^a) and all these kinds of things?
>
...
>
> This was all 4D, but the horror starts in higher dimensions. There γ
