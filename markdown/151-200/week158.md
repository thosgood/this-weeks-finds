...
["Week 118"](#week118)):
    ...
    L = - (1/2k2) e R  
        - (1/2) e \psi*M ΓMNP DN[ω + ω')/2] \psiP 
        - (1/48) e F2
        - (√2k/384) e (\psi*M ΓMNPQRS \psiS + 12 \psi*N ΓPQ \psiR) (F + F')NPQR 
...
Ricci scalar cooked up from the spin connection. "\psi~N~" is the
gravitino field, and physicists write the inner product on spinors as
"\psi*~N~ \psi^N^", where the star should really be a bar: a line drawn
over the letter \psi. "A" is the 3-form field and "F" is the field
...
> \psi_M with a Γ\^M matrix and summing of course on the vector index. The
...
>     >
>     > L = - (1/2k^2) e R
>     >     - (1/2) e psibar_M Γ^{MNP} D_N[(ω + ω')/2] \psi_P
>     >     - (1/48) e F^2
>     >     - (sqrt(2)k/384) e
>     >       (psibar_M Γ^{MNPQRS} \psi_S + 12 psibar^N Γ^{PQ} \psi^R)
>     >       (F + F')_{NPQR}
...
> positions (remember, for anticommuting variables \psi\^a φ_a = - \psi_a
