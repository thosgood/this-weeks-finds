...
This reduces to the usual derivative as q → 1. Note that the
...
the line, say ψ. The position operator Q multiplies a wavefunction by x:
...
and make the quantum effects small by letting ħ → 0, or make the
gravitational effects go away by setting q → 1. The problem is, I\'ve
